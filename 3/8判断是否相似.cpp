...
/*
问题：判断两棵树是否相似，相似返回Ture，不相似返回False
	  相似：形态相同就行
...
#define True 1;
#define False 0;
int isSim(BiTree T1,BiTree T2)
{
	if(T1==NULL &&T2==NULL)//出口
	{
		return True;
	}
	...
	{
		return isSim(T1->lchild,T2->lchild)&&isSim(T1->rchild,T2->rchild);
	}
}
...
int main()
{
	...
	char preOrderList2[] = {'A', 'B', 'D', 'E', 'C', 'F'};
	int preOrderListLength2 = 6;

	char inOrderList2[] = {'D', 'B', 'E', 'A', 'F', 'C'};
	int inOrderListLength2 = 6;

	BiTree T2 = creatBiTree(preOrderList2, 0, preOrderListLength2 - 1,
