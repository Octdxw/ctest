...
算法思想：
	使用层次遍历
	不管节点是否为空，都押入队列然后出队列，
	出队列时，如果发现空节点，队列中还存在实节点，那么就是非完全二叉树，
	反之，就是完全二叉树
...
int isCompleteBiTree(BiTree T)
{
	...
	while(isEmptyQueue(Q) != 1) //队列不为空
	{
		...
		if(e==NULL)//发现有一个节点为空
		{
			break;//跳出循环，判断其后面是否还有实节点
		}
		enQueue(Q,e->lchild);//节点e不为空，压入其左孩子
		enQueue(Q,e->rchild);//节点e不为空，压入其右孩子
	}
