...
#define WHITE 0 //顶点没有被访问
#define RED 1 //顶点正在被访问，在递归过程中
#define BLACK 2 //顶点已经被访问完成，退出递归

int colorVisit[MAX_VERTEX_NUM] = {0};

int getRing(ALGraph G, int v)
{
	if(colorVisit[v] == RED)//找到环
	{
		return 1;
	}
	else
	{
		colorVisit[v] = RED; //v正在被访问
		ArcNode *nextArcNode = G.vertices[v].firstArc; //v的邻居节点
		while(nextArcNode != NULL)
		{
			int neiborVertex = nextArcNode->adjvex;
			int ret = 0;//还没发现环
			if(colorVisit[neiborVertex] != BLACK)
			{
				ret = getRing(G, neiborVertex);
			}

			if(ret == 1) //通过DFS邻居节点，找到了环
			{
				return 1;
			}
			else//通过DFS邻居节点，没找到环，让下一个邻居节点进入循环
			{
