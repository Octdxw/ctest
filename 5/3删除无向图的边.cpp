...
int deleteOneArc(int src, int dest, ALGraph *G)
{
	...
	{
		G->vertices[src].firstArc = nextArcNode->nextArc;
	}

	//dest不在源点之后
	ArcNode *preNextNode = nextArcNode; //源点之后的第一个节点
	nextArcNode = nextArcNode->nextArc; //源点之后的第二个节点

	while(nextArcNode != NULL)
	{
		if(nextArcNode->adjvex == dest)
		{
			preNextNode->nextArc = nextArcNode->nextArc;
		}
		//如果nextArcNode不是dest，继续往后找
		preNextNode = nextArcNode;
...
int deleteArc(int src, int dest, ALGraph *G)
{
	...
	if(G->kind == UDG)
	{
		deleteOneArc(src, dest, G);
		deleteOneArc(dest, src, G);
	}
	return 1;
}
